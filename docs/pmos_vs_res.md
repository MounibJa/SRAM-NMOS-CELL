...
- While the idea of the PMOS is to replace the 50K resistor, there are key differences that explain the tradeoffs of each design. The PMOS operates in the triode region, though it does creep into cutoff area. The triode region models the resistor almost 1:1, however the cutoff part is where it tapers away from it. This is actually a positive difference in many ways, though there are drawbacks to consider as well (later deemed unimportant). Look below for the current vs node voltage characteristics of the pmos vs the resistor.
